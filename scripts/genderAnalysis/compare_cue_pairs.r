...
# example: hermoso/a
# nPair = 626
X.cue_pairs.normalized %>% filter(cue_nPair==626,response!="agradable") %>% 
  ungroup() %>% 
  select(cue_nPair,response,R123,cue_gender,cue_POS,cue_lemma) %>% 
  group_by(cue_nPair,response,cue_POS) %>% 
  pivot_wider(names_from = cue_gender,values_from=R123,values_fill=0) %>%
  #filter(response %in% cues$cue) %>% #keep only responses that are in the cue list
  filter(Masc>2 | Fem>2) %>% 
  pivot_longer(c(Masc,Fem),names_to = "cue_gender",values_to = "R123") %>% 
  arrange(response) %>% 
  pivot_wider(names_from = response,values_from = R123) %>% 
  ungroup() %>% 
  select(-starts_with("cue")) %>% 
  htmlTable::htmlTable(rnames = c("hermoso","hermosa"),cgroup = "Normalized responses")

X.cue_pairs %>% filter(cue_nPair==626,response!="agradable") %>% 
  ungroup() %>% 
  select(cue_nPair,response,R123,cue_gender,cue_POS,cue_lemma) %>% 
  group_by(cue_nPair,response,cue_POS) %>% 
  pivot_wider(names_from = cue_gender,values_from=R123,values_fill=0) %>%
  ...
  ungroup() %>% 
  select(-starts_with("cue")) %>% 
  htmlTable::htmlTable(rnames = c("hermoso","hermosa"),cgroup = "Original responses")
