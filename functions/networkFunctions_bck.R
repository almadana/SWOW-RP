...
getEntropy <- function(X,type){
  ...
  if(!"Freq" %in% colnames(X)){
    X  = X %>% filter(complete.cases(response)) %>% group_by(cue,response)  %>% 
      summarise (Freq = n(), .groups = 'drop')
  }
  
  X$cue           = factor(X$cue)
  X$response      = factor(X$response)
  S               = sparseMatrix(i = as.numeric(X$cue),
                                 j = as.numeric(X$response), x = X$Freq)
  
  switch(type,
         cues = {
           H           = unlist(pbapply(S,1,entropy,simplify = F))
           labels      = X %>% group_by(cue) %>% summarise()
         },
         response ={
           H          = unlist(pbapply(S,2,entropy,simplify = F))
