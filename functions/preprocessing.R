...
normalizeCues = function(X,file.cueCorrections){
  X.cuecor  = read.csv(file.cueCorrections,stringsAsFactors = FALSE,sep='\t',
                       encoding = 'UTF-8') %>% as_tibble()
  
  X = left_join(X,X.cuecor,by = c('cue'='original'))
  nCues = X %>% filter(cue!=correction) %>% group_by(cue) %>% tally() %>% nrow()
  X = X %>% mutate(cue = ifelse(!is.na(correction),correction,cue)) %>%
    select(-correction)
...
spellCheckResponse = function(X,file.lexicon,
  ...
  X.cueResp = read.csv(file.cueResponseCorrections, stringsAsFactors = FALSE,
                       strip.white = TRUE,sep = '\t',encoding = 'UTF-8') %>%
    as_tibble()
  # Temporary fix
